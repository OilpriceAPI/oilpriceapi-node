...
describe("DieselResource", () => {
  ...
  describe("getStations()", () => {
    ...
    it("should reject invalid latitude (too high)", async () => {
      await expect(client.diesel.getStations({ lat: 91, lng: -122, radius: 5000 })).rejects.toThrow(
        "Latitude must be between -90 and 90",
      );
    });

    it("should reject invalid longitude (too low)", async () => {
      await expect(client.diesel.getStations({ lat: 37, lng: -181, radius: 5000 })).rejects.toThrow(
    ...
    it("should reject negative radius", async () => {
      await expect(client.diesel.getStations({ lat: 37, lng: -122, radius: -100 })).rejects.toThrow(
    ...
    it("should pass correct params to request()", async () => {
      const requestSpy = vi.spyOn(client as any, "request").mockResolvedValue({
        regional_average: { price: 3.89 },
        stations: [],
        search_area: { center: { lat: 37, lng: -122 }, radius_meters: 5000, radius_miles: 3.1 },
